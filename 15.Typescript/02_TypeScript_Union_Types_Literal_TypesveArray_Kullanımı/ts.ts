...
let value: number | string; // 'value' değişkeni ya sayı ya da string alabilir
...
let direction: "left" | "right"; // 'direction' değişkeni yalnızca "left" veya "right" olabilir
...
let numbers: number[] = [1, 2, 3, 4]; // numbers dizisi sadece sayılar içerebilir
...
let fruits: string[] = ["apple", "banana", "cherry"]; // fruits dizisi yalnızca string değerler alabilir
console.log("fruits array:", fruits);

// Alternatif olarak Array<T> kullanımı
let mixedNumbers: Array<number> = [1, 2, 3, 4, 5]; // Aynı şey Array<number> ile de yapılabilir
...
let squaredNumbers = numbers.map((num) => num * num); // her sayıyı karesine alalım
console.log("squaredNumbers:", squaredNumbers);

// Dizilerde, opsiyonel elemanlar (Optional elements in arrays)
let optionalArray: (string | number)[] = [1, "apple", 2, "banana"];
...
let user: [string, number] = ["Alice", 30]; // İlk eleman string, ikinci eleman number
